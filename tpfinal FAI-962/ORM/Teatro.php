...
class Teatro
{
    ...
    public function getColFunciones()
    {
        ...
        $colFunciones = array_merge($colCine, $colMusical, $colObra);   //Combina los elementos de uno o más arrays juntándolos de modo  que los valores de uno se anexan al final del anterior. Retorna el array resultante.*/
    ...
    /**
     * Recupera los datos de un teatro por su id
     * @param string
    ...
    public function Buscar($idT)
    {
        ...
        if ($base->Iniciar()) {                        //devuelve true/false si se pudo establecer conexion con la base de datos
            if ($base->Ejecutar($consultaTeatro)) {   //ejecuta la consulta y me devuelve si se pudo concretar o no (true o falSe)
                if ($row2 = $base->Registro()) {     //me devuelve una array asociativo con la tupla de la tabla en la que esta apuntando el cursor en ese momento, luego avanZa al siguiente punto. 
        ...
        if ($base->Iniciar()) {
            if ($base->Ejecutar($consultaTeatro)) {
                ...
                while ($row2 = $base->Registro()) {         // row2 es un arreglo asociativo q corresponde a una tupla de la tabla
                    ...
                    array_push($arregloTeatro, $teatro);     //agrega un nuevo objeto a la coleccion de teatros
                }
            } else {
                $this->setmensajeoperacion($base->getError());
            }
